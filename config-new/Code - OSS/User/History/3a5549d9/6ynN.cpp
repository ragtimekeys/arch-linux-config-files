...
/*
1. com->js initiatefirmwareupdate (chip, "3.55")
...
nitty gritty binary
FirmwareUpdater <- JamstikHandler : whattosendtothejamstik,percentagefinishedsofar interpretUpdateSysex(whatthejamstiksenttothecomputer)

struct InterpretUpdateSysexReturn {
...
void FirmwareUpdater::downloadFirmwareFilesForVersionNumber(std::string versionNumber, std::string directoryToSaveFiles)
{
...
void FirmwareUpdater::initiateFirmwareUpdate(int chip, const std::string &mostUpToDateFirmwareVersionFromServer, const std::string &directoryForFirmwareFiles)
{
    ...
    m_selectedFirmwareFileFullPath = chip == 2 ? m_chip2FullPath : m_chip1FullPath;

    m_firmwareFile.open(chip == 2 ? fullPathAndFileNameForHexChip2 : fullPathAndFileNameForCyacdChip1);
    ...
    } else /*basically if chip == 1*/ {
        m_numberOfLinesInCyacdFirmwareFile = countLines(fullPathAndFileNameForCyacdChip1);
    }
    int sizeOfFileToUpdate = chip == 2 ? m_numberOfLinesInHexFirmwareFile : m_numberOfLinesInCyacdFirmwareFile;
    ...
    FirmwareUpdater::sendMessage("sendMidi", convertVectorToString(updateArray));
}
...
{
    std::cout << "cyacdtobincalled" << std::endl;
...
std::vector<std::vector<int>> FirmwareUpdater::createLinesToSendBack(const std::vector<int> &lineJamstikGave, const std::string &typeOfJamstikRaw, int chip)
{
    ...
    int numOfLinesForCurrentFirmwareFile = chip == 2 ? m_numberOfLinesInHexFirmwareFile : m_numberOfLinesInCyacdFirmwareFile;
...
{
    std::cout << "interpretUpdateSysex called" << std::endl;
    if (data[4] == 0x54)
    {
        ...
        {
            chip = 1;
            initiateFirmwareUpdate(1, savedMostUpToDateFirmwareVersionFromServer, "/Users/jimmy/");
        }
        else
        {
            FirmwareUpdater::sendMessage("firmwareUpdateFinished", "YAY");
        }
        return;
    }
    ...
    for (size_t i = 0; i < dataToSend.size(); i++)
    {
        ...
        if (dataToSend[i].size() > 255 && false)
        {
            for (size_t j = 0; j < std::ceil(dataToSend[i].size() / 255.0); j++)
            {
                ...
                FirmwareUpdater::sendMessage("sendMidi", jsonChunkToSend.dump());
            }
        }
        else
        {
            nlohmann::json jsonDataToSend(dataToSend[0]);
            FirmwareUpdater::sendMessage("sendMidi", jsonDataToSend.dump());
        }
    }
}