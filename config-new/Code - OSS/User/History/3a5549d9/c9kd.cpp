...
{
    std::cout << "FirmwareUpdater::FirmwareSendBlock() -> " << messageType << " ..... " << messageInfo << std::endl;
}
...
void FirmwareUpdater::addLineToFile(const std::string& filename, const std::string& content) {
    ...
    if (file.is_open()) {
        file << content;
        file.close();
        std::cout << "FirmwareUpdater::addLineToFile() -> Line added" << std::endl;
    }
    else {
        std::cout << "FirmwareUpdater::addLineToFile() -> Unable to open file" << filename << std::endl;
    }
}
...
bool FirmwareUpdater::downloadFile(const std::string& url, const std::string& outputPath) {
    ...
    if (curl) {
        ...
        if (outputFile.is_open()) {
            ...
            outputFile.close();
            std::cout << "FirmwareUpdater::downloadFile() -> File downloaded successfully." << std::endl;
            return true;
        }
        else {
            std::cerr << "FirmwareUpdater::downloadFile() -> Failed to open output file." << std::endl;
        }
    }
    else {
        std::cerr << "FirmwareUpdater::downloadFile() -> Failed to initialize libcurl." << std::endl;
    }
    return false;
}

// saves both hex and cyacd files for a given firmware version
void FirmwareUpdater::FirmwareFilesDownloadForVersionNumber(std::string versionNumber, std::string directoryForFirmwareFiles)
{
    m_directoryForFirmwareFiles = directoryForFirmwareFiles;
    for (int chip = 1; chip <= 2; chip++) {
        std::string chosenChipAsName = chip == 2 ? "dsp/" : "main/";
        std::string fileNameAndPath = directoryForFirmwareFiles + versionNumber + (chip == 2 ? ".hex" : ".cyacd");
        std::string url = "https://linux.jamstik.com/files/firmware/raw/Studio/" + chosenChipAsName + versionNumber + "." + (chip == 2 ? "hex" : "cyacd");
        std::cout << "FirmwareUpdater::FirmwareFilesDownloadForVersionNumber() -> url is: " << url << std::endl;
        if (downloadFile(url, fileNameAndPath)) {
            //add empty 0 to the end
            //addLineToFile(fileNameAndPath, ":0");
            std::cout << "FirmwareUpdater::FirmwareFilesDownloadForVersionNumber() -> File downloaded and saved successfully." << std::endl;
        }
    }
}
...
int FirmwareUpdater::countLines(const std::string& filename) {
    ...
    if (file.is_open()) {
        ...
        std::cout << "FirmwareUpdater::countLines() -> Unable to open file: " << filename << std::endl;
    }
...
void FirmwareUpdater::FirmwareUpdateBegin(int chip, const std::string &mostUpToDateFirmwareVersionFromServer, const std::string &directoryForFirmwareFiles) {
    ...
    std::cout << "FirmwareUpdater::FirmwareUpdateBegin()" << std::endl;
    ...
    if (m_chip == 2) {
        ...
        //additionally, if chip is 2, we know that the firmware update has started from the beginning
        FirmwareUpdateAlert(false);
    } else /*basically if m_chip == 1*/ {
        m_numberOfLinesInCyacdFirmwareFile = countLines(fullPathAndFileNameForCyacdChip1);
    }
    //IMPORTANT: I put + 1 here to address that legacy bug that requires sending an empty line for the last thing.
    //need to make sure it works properly!
    int sizeOfFileToUpdate = m_chip == 2 ? m_numberOfLinesInHexFirmwareFile + 1 : m_numberOfLinesInCyacdFirmwareFile + 1;
...
std::string FirmwareUpdater::readSpecificLine(const std::string& filename, int lineNumber) {
    std::cout << "FirmwareUpdater::readSpecificLine() -> filename: " << filename << ", line number: " << lineNumber << std::endl;
...
void FirmwareUpdater::FirmwareUpdateAlert(bool finished) {
    std::cout << "FirmwareUpdateAlert(), finished? ->" << finished << std::endl;
}
void FirmwareUpdater::FirmwareUpdateCancel() {
    std::cout << "FirmwareUpdateCancel()" << std::endl;

    //call a function from the service
    //ZivixService::CancelUpdate();

    //TODO: should we also call alert?
    //FirmwareUpdater::FirmwareUpdateAlert(false);
...
void FirmwareUpdater::FirmwareGetBlock(const std::vector<int> &data, std::string savedMostUpToDateFirmwareVersionFromServer) {
    ...
    if (data[4] == 0x54) {
        ...
        if (m_chip == 2) {
            m_chip = 1;
            FirmwareUpdateBegin(1, savedMostUpToDateFirmwareVersionFromServer, m_directoryForFirmwareFiles);
        }
        else {
