...
public:
    ...
    void FirmwareFilesDownloadForVersionNumber(std::string versionNumber, std::string directoryForFirmwareFiles);
    ...
    void FirmwareGetBlock(int startingLine, int numOfLines);
    void FirmwareUpdateCancel(); //TODO: this may have a callback from jamstik
    
    //to be triggered based on interal class code
    void FirmwareSendBlock(std::vector<char> 8BitBytes, const std::string &messageInfo);  //messageType is just placeholder
    void FirmwareUpdatePercentage(int percentage);
...
private:
    ...
    std::string m_directoryForFirmwareFiles = "";
